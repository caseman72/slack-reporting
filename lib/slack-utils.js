...
SlackUtils.prototype = {
  ...
  _http_get: function(url, callback, no_cache) {
    ...
    // TODO: fix base dir in this.config
  ...
  _parse_blockers: function(list) {
    ...
    (list || []).forEach(function(item) {
      (item.fields || []).forEach(function(field) {
        if (/Blocker/i.test(field.title) && !/None|On Track/i.test(field.value)) {
  ...
  daily: function(room_or_id, date/*, callback*/) {
    ...
    Object.keys(config.members).forEach(function(key) {
      if (config.members[key].report) {
        reporting_members[key] = Object.assign({},
          config.members[key],
          {reported: false, today: ""}
        );
      }
    });
      ...
      if (payload && payload.ok && payload.messages && payload.messages.length) {
        payload.messages.forEach(function(message) {
          ...
          if (isDefined(m) && !m.reported && /Status Update/i.test(message.text)) {
            var today = self._parse_today(message.attachments);

            console.log(Dt.h_mm_ampm(+message.ts), "-", m.handle, "<{0}>".format(m.real_name || m.email))
        ...
        payload.messages.forEach(function(message) {
          ...
          if (isDefined(m) && !m.reported && /Update.*\n/i.test(message.text)) {
            ...
            console.log(m.handle, "<{0}>".format(m.real_name || m.email))
            console.log(today.join("\n"));
            console.log("");
