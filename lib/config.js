...
try {
  config.prefs = require(prefs_file);
}
catch (e) {
...
Object.keys(config)
  .map(function(prop) {
    ...
    if (_.isDefined(config.prefs[prop])) {
      config[prop] = config.prefs[prop];
    }
    // reporting level of rc file
    if (_.isDefined(config.prefs.reporting) && _.isDefined(config.prefs.reporting[prop])) {
      config[prop] = config.prefs.reporting[prop];
    }
